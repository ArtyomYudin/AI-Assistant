...
class RAGCore:
    ...
    def embeddings(self):
        if self._embeddings is None:
            ...
            self._embeddings = OpenAIEmbeddings(model=self.config.EMBEDDING_NAME, api_key="EMPTY", base_url=self.config.EMBEDDING_BASE_URL, embedding_ctx_length=512, timeout=60)
    ...
    def llm(self):
        if self._llm is None:
            ...
            self._llm = ChatOpenAI(model=self.config.LLM_NAME, api_key="EMPTY", base_url=self.config.LLM_BASE_URL, max_tokens=4096, temperature=0.7, streaming=True, extra_body={"chat_template_kwargs": {"enable_thinking": False}})
        return self._llm
    ...
    def setup_vectorstore(self, documents: List[Document]) -> None:
        ...
        if dense_dim <= 0:
            ...
            return
        self.milvus.create_collection_if_needed(dense_dim=dense_dim)
        rows = [{"text": t, "source": s, "hash": h or "", "dense_vector": dv} for t, s, dv, h in zip(texts, sources, dense_vectors, hashes)]
        unique = self.milvus.ensure_not_duplicate_rows(rows)
        if not unique:
        ...
        for i in range(0, len(unique), bs):
            total += self.milvus.insert_records(unique[i:i+bs])
    ...
    def create_retriever(self, k: Optional[int] = None, fetch_k: Optional[int] = None) -> None:
        ...
        def retrieve(query: str) -> List[Document]:
            ...
            if not self.milvus.client.has_collection(self.milvus.collection_name):
                return []
            try:
                ...
                return []
            return self.milvus.hybrid_search(query_text=query, query_dense=dense, fetch_k=fetch_k, top_k=k, reranker_endpoint=self.config.RERANKER_BASE_URL)
